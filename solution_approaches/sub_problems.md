...
Also, for some optimizers (e.g. Scipy's SLSQP) it is not safe to nest multiple copies within each other --- the code is non re-entrant, which means the multiple instances will stomp on each other. 
