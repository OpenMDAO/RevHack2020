...
def change(atov, v_inf, theta, T, alpha_stall, CD0, AR, e, rho, S, m, a0, t_over_c, coeffs, v_i, v_factor, Normal_F):
    """
    ...
    delta_xdot = (T * np.sin(theta) - D_fuse * np.sin(atov) - D_wings * np.sin(theta + aoa_blown) - L * np.cos(theta + aoa_blown) - Normal_F * np.cos(theta)) / m 
    delta_ydot = (T * np.cos(theta) - D_fuse * np.cos(atov) - D_wings * np.cos(theta + aoa_blown) + L * np.sin(theta + aoa_blown) + Normal_F * np.sin(theta) - m * 9.81) / m 
...
class Dynamics(ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input('powers', val=np.ones(num_nodes)) # control
        self.add_input('thetas', val=np.ones(num_nodes)) # control
        # openmdao outputs from the component
        
        # state history inputs that some calcs need
        self.add_input('x_dot', val=np.ones(num_nodes))
        self.add_input('y_dot', val=np.ones(num_nodes)) 
        
        self.add_output('v_x', val=np.ones(num_nodes)) # state rates for x
        self.add_output('v_y', val=np.ones(num_nodes)) # state rates for y
        ...
        self.add_output('power', val=np.ones(num_nodes)) # state rates for energy
        ...
        self.add_output('thrusts', val=np.ones(num_nodes)) 
    ...
    def compute(self, inputs, outputs):
        ...
        for i in range(self.options['num_nodes']):
            ...
            power = inputs['powers'][i]
            theta = inputs['thetas'][i]
            x_dot = inputs['x_dot'][i]
            y_dot = inputs['y_dot'][i]
            ...
            step = change(atov, v_inf, theta, thrust, self.alpha_stall, self.CD0, self.AR, self.e, self.rho, self.S, self.m, self.a0, self.t_over_c, self.quartic_poly_coeffs, vi, self.v_factor, self.n_props * Normal_F)
            ...
            outputs['v_x'][i] = inputs['x_dot'][i]
            outputs['v_y'][i] = inputs['y_dot'][i]

            outputs['a_x'][i] = step[0]
            outputs['a_y'][i] = step[1]
            
            outputs['power'][i] = power
